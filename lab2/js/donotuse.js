...
// Example: Using makeColorScaleNBJ
var colorScale = makeColorScaleNBJ(csvData);
...
// Define your color scale generation functions
function makeColorScaleNBJ(data) {
    var colorClasses = [
        "#D4B9DA",
        "#C994C7",
        "#DF65B0",
...
function makeColorScaleEE(data) {
    var colorClasses = [
        "#D4B9DA",
        "#C994C7",
        "#DF65B0",
...
// Update the dropdown creation functions
function createDropdown(csvData){
    ...
    function createSchemeDropdown() {
        var dropdown = d3.select("body")
            .append("select")
            .attr("class", "scheme-dropdown")
            .on("change", function(){
                changeScheme(this.value, csvData)
            });
        ...
        var schemes = ["Natural Breaks (Jenks)", "Quantile", "Equal Interval"]; // Add your scheme options here
    
        var schemeOptions = dropdown.selectAll("schemeOptions")
            .data(schemes)
            .enter()
            .append("option")
            .attr("value", function(d){ return d })
...
// Update changeAttribute function to handle attribute changes
function changeAttribute(attribute, csvData, scheme) {
    expressed = attribute;
    var colorScale;

    // Check the selected scheme and assign the appropriate color scale function
    if (scheme === "Natural Breaks (Jenks)") {
        colorScale = makeColorScaleNBJ(csvData);
    } else if (scheme === "Quantile") {
        colorScale = makeColorScaleQ(csvData);
    } else if (scheme === "Equal Interval") {
        colorScale = makeColorScaleEE(csvData);
    }

    // Check if colorScale is a function before using it
    if (typeof colorScale === 'function') {
        ...
        console.error("Color scale is not a function.");
    }
};



// Bind changeAttribute to attribute dropdown change event
d3.select(".dropdown").on("change", function() {
    changeAttribute(this.value, csvData);
});

// Update changeScheme function to handle scheme changes
function changeScheme(scheme, csvData) {
    if (scheme === "Natural Breaks (Jenks)") {
        var colorScale = makeColorScaleNBJ(csvData);
    } else if (scheme === "Quantile") {
        var colorScale = makeColorScaleQ(csvData);
    } else if (scheme === "Equal Interval") {
        var colorScale = makeColorScaleEE(csvData);
    }
    // Update the visualization based on the selected scheme
    var metros = d3.selectAll(".metros")
        .transition()
        .duration(1000)
        .style("fill", function(d) {
...
// Bind changeScheme to scheme dropdown change event
d3.select(".scheme-dropdown").on("change", function() {
    changeScheme(this.value, csvData);
});
 //end of changeAttribute()

function changeScheme(scheme, csvData){
    // Handle changes for classification scheme selection
    if (scheme === "Scheme 1") {
        var colorScale = makeColorScaleNBJ(csvData);
        // Update visualization with colorScale for Scheme 1
    } else if (scheme === "Scheme 2") {
        var colorScale = makeColorScaleQ(csvData);
        // Update visualization with colorScale for Scheme 2
    } else if (scheme === "Scheme 3") {
        var colorScale = makeColorScaleEE(csvData);
        // Update visualization with colorScale for Scheme 3
    }
    // Update the visualization based on the selected scheme
}
